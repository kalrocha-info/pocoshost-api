...
function publicUser(user) {
  const { password_hash, email_verification_token_hash, password_reset_token_hash, ...safeUser } = user;
  return safeUser;
}

function normalizeDocumentNumber(documentNumber) {
...
async function issuePasswordResetEmail(user, client = pool) {
  const token = createVerificationToken();
  const tokenHash = hashToken(token);
  await client.query(
    `UPDATE users
...
export async function register(req, res) {
  ...
  try {
    if (assignedRole === 'host') {
      ...
      const normalizedDocument = normalizeDocumentNumber(document_number);
      ...
      const existingDocument = await pool.query(
        `SELECT id
           FROM users
          WHERE regexp_replace(COALESCE(document_number, ''), '\\D', '', 'g') = $1
    ...
    const normalizedDocument = assignedRole === 'host' ? normalizeDocumentNumber(document_number) : document_number;
...
export async function requestPasswordReset(req, res) {
  ...
  try {
    ...
    res.json({ success: true, message: 'Se houver uma conta ativa para este e-mail, enviaremos instruções para redefinir a senha.' });
  } catch (err) {
    sendServerError(res, err);
  }
}
  ...
  try {
    const result = await pool.query(
      `SELECT id, full_name, email, role, avatar_url, phone, document_type,
              document_number, company_name, address_info, email_verified, account_status, created_date
  ...
  try {
    const result = await pool.query(
      `UPDATE users SET full_name = COALESCE($1, full_name),
       ...
       RETURNING id, full_name, email, role, avatar_url, phone, email_verified, account_status`,
...
export async function deleteMe(req, res) {
  ...
  try {
    ...
    const active = await client.query(
      `SELECT
        (SELECT COUNT(*) FROM reservations WHERE guest_id = $1 AND status IN ('pending', 'approved', 'confirmed')) +
        (SELECT COUNT(*) FROM reservations r JOIN properties p ON p.id = r.property_id
          WHERE p.created_by = $1 AND r.status IN ('pending', 'approved', 'confirmed')) AS count`,
      [req.user.id]
    );
