  ...
  try {
    ...
    if (r.status === 'pending' && new Date(r.expires_at).getTime() <= Date.now()) {
      ...
      return res.status(409).json({
        error: 'O prazo de 30 minutos para pagamento expirou. Faça uma nova reserva.',
      });
    }

    if (r.status !== 'pending') {
      await client.query('ROLLBACK');
      return res.status(409).json({ error: 'Esta reserva não está disponível para pagamento.' });
    }
