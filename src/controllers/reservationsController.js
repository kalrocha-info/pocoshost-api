...
export async function checkAvailability (req, res) {
  ...
  if (!property_id || !check_in || !check_out) { return res.status(400).json({ error: 'property_id, check_in e check_out são obrigatórios.' }) }
  try {
...
export async function create (req, res) {
  ...
  if (!property_id || !check_in || !check_out || !guests) { return res.status(400).json({ error: 'property_id, check_in, check_out e guests são obrigatórios.' }) }
...
export async function updateStatus (req, res) {
  ...
  if (!allowed.includes(status)) { return res.status(400).json({ error: `Status deve ser: ${allowed.join(', ')}` }) }
