...
describe('AUTH — /api/auth', () => {
  ...
  describe('POST /register', () => {
    ...
    it('rejeita registo sem full_name', async () => {
      const res = await request(app).post('/api/auth/register').send({
        email: 'semnome@example.test', password: 'senha123',
      });
      expect(res.status).toBe(400);
    });
    ...
    it('rejeita email duplicado', async () => {
      await request(app).post('/api/auth/register').send({
        full_name: 'Primeiro', email: 'dup@example.test', password: 'senha123',
      });
      const res = await request(app).post('/api/auth/register').send({
        full_name: 'Segundo', email: 'dup@example.test', password: 'outrasenha',
      });
    ...
    it('não expõe password_hash na resposta', async () => {
      const res = await request(app).post('/api/auth/register').send({
        full_name: 'Seguro', email: 'seguro@example.test', password: 'senha123',
      });
      expect(res.body.user.password_hash).toBeUndefined();
    });
  });
  ...
  describe('Verificação de e-mail', () => {
    it('ativa a conta com token enviado por e-mail e permite login', async () => {
      await request(app).post('/api/auth/register').send({
        full_name: 'Verificar Email', email: 'verificar@example.test', password: 'senha123',
      });
      ...
      const login = await request(app).post('/api/auth/login').send({
        email: 'verificar@example.test', password: 'senha123',
      });
    ...
    it('reenvia link de verificação sem revelar se a conta existe', async () => {
      await request(app).post('/api/auth/register').send({
        full_name: 'Reenvio', email: 'reenvio@example.test', password: 'senha123',
      });
      verificationEmails.length = 0;
  ...
  describe('POST /login', () => {
    ...
    it('bloqueia login enquanto o e-mail não foi verificado', async () => {
      await request(app).post('/api/auth/register').send({
        full_name: 'Pendente', email: 'pendente@example.test', password: 'senha123',
      });
      const res = await request(app).post('/api/auth/login').send({
        email: 'pendente@example.test', password: 'senha123',
      });
